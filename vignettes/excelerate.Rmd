...
We will use `create_meta()` to add comments on each of the columns in these dataframes with a description.
This metadata will be presented in the README sheet of the Excel file.

The `create_meta()` function requires the `results` argument to be a dataframe and the `colname_descriptions` argument to be a named character vector where the names are the column names and the values are the descriptions.

```{r}
iris_commented <- create_meta(results = iris,
